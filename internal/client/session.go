...
//
// When the session was opened with keepConn=true (PersistentSender), the
// underlying WebSocket is left open so the next pairing can reuse it; the
// PersistentSender owns the conn lifetime in that case.
